...
async def tx_serializer_model(dut, NB_DATA, collected, expected_count):
    """
...
async def test_fft16_stage1(dut):
    ...
    while len(rtl_stage1) < N and capture_cycles < 5000:
    ...
    while len(rtl_mdc) < N and capture_cycles < 5000:
    ...
    # =====================================================
    # Buffer output capture (with tx_serializer emulation)
    # =====================================================

    collected_buffer = []
    cocotb.start_soon(tx_serializer_model(dut, NB_DATA, collected_buffer, N))

    timeout_cycles = 50000
    elapsed = 0
    while len(collected_buffer) < N and elapsed < timeout_cycles:
        await RisingEdge(dut.i_clk)
        elapsed += 1

    assert len(collected_buffer) == N, \
        f"Buffer capture timeout: only got {len(collected_buffer)} of {N} samples."
    ...
    buffer_ord = [0j] * N
    for i in range(N):
        buffer_ord[mdc_map[i]] = collected_buffer[i]
    ...
    for idx in range(N):
        ...
        assert diff_re < tol and diff_im < tol, f"Stage 1 mismatch at index {idx}"
    ...
    for idx in range(N):
        ...
        assert diff_re < tol and diff_im < tol, f"MDC stage mismatch at index {idx}"
    ...
    for idx in range(N):
        ...
        assert diff_re < tol and diff_im < tol, f"Final output mismatch at index {idx}"
    ...
    for idx in range(N):
        ...
        assert diff_re < tol and diff_im < tol, f"Buffer output mismatch at index {idx}"
