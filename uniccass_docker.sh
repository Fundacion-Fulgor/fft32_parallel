...
current RTL, nothing is copied and you cannot forget to sync.

The container is kept between sessions under the name '$CONTAINER'. That is
deliberate: 'make fft16_project' builds LibreLane through nix-shell the first
time, which takes a long while, and a persistent container means it happens
...
Once inside:
  cd unic_cass_wrapper_user_project && make fft16_project
EOF
}
...
update_sources() {
    ...
    log "librelane ..... $(git -C "$PROJECT_DIR/librelane" describe --tags 2>/dev/null || echo '?')"
    log "IHP-Open-PDK .. $(git -C "$PROJECT_DIR/IHP-Open-PDK" describe --tags 2>/dev/null || echo '?')"
    if grep -q '"VDD_PIN_VOLTAGE"' "$PROJECT_DIR/librelane/librelane/config/flow.py" 2>/dev/null; then
        warn "This librelane revision requires PDK variables the current IHP PDK no"
        warn "longer provides; 'make' will fail. Move it to 3.0.0rc1 or newer."
    fi
